...
for (let i = 0; i < results.length; i++) {
    if (results[i].order.customerId == "ERNSH") {
        result1.push(results[i].order)
    }    
}
console.log(result1);
...
results.filter(result =>{
  console.log(result.order.employeeId)
})
...
results.filter(result => {
  if (result.orderDetails.length > 3) {
